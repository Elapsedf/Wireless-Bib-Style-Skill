...
git clone https://github.com/Elapsedf/Wireless-Bib-Style-Skill.git ~/.codex/skills/ieee-bib-style-audit
```
...
```bash
mkdir -p ~/.codex/skills
cp -R Wireless-Bib-Style-Skill ~/.codex/skills/ieee-bib-style-audit
```
...
# Wireless Bib Style Skill 中文说明

## 描述

Wireless Bib Style Skill 是一个 Codex skill，用来按照无线通信论文常用的 IEEE 参考文献模板检查和轻量规范 BibTeX 文件。它可以帮助统一作者姓名缩写、IEEE 期刊缩写、会议 `Proc.` 写法、月份和地点字段、arXiv 条目格式，以及重复 citation key。
